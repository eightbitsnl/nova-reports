...
trait Reportable
{
	...
	public static function withRelations($relations = null)
    {
		return empty($relations)
			? parent::query()
			: (($relations === true)
				? parent::with(static::getReportableRelations())
	...
	public static function getReportableRelationsWithFields()
	{
		...
		return collect($relations)->mapWithKeys(function($relation){
	...
	public static function getExportableFields($includeRelations = true)
	{
		...
		if($includeRelations)
		{
			$result = array_merge($result,
				collect(static::getReportableRelationsWithFields())->mapWithKeys(function($v,$k){
					return [$k => [
						'type' => 'relation',
						'fields' => $v
					]];
